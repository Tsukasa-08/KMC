...
filepaths = glob.glob(os.path.join(search_dir_path, '**/modified_mean_*.csv'), recursive=True)
...
for filepath in filepaths :
    ...
    tmp = pd.read_csv(filepath, index_col=[0,1], comment="#")
...
if len(data_list) == 0:
    ...
    if (dataset.to_csv(os.path.join(search_dir_path, "concat_modified_mean_displacement.csv"))) :
        print("file saved at ", os.path.join(search_dir_path, "concat_modified_mean_displacement.csv"))
